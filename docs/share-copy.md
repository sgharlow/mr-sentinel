...
**Demo video URL:** https://youtu.be/S93xnolHRe0
...
| "How does it use Agent Builder / MCP?" | "It's a Google ADK agent (Gemini 2.5 Flash on Vertex AI) that reads each MR through GitLab's MCP server (`@zereight/mcp-gitlab` via ADK `MCPToolset`) and records a structured verdict via a `record_verdict` tool. Write-backs use GitLab REST because the official Duo MCP server can't post MR notes or labels — honest hybrid, documented in `docs/mcp-endpoint-audit.md`." |
...
Architecture: a 3-min walkthrough on YouTube — https://youtu.be/S93xnolHRe0
